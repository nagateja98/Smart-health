...
opt_model.addConstr(
        ...
        rhs=0, 
        name="Cons3_{0}_{1}".format(i,j))
    for i in Cities for j in Facilities: Cost[i,j] >= 500}




objective = grb.quicksum(Fixed_Cost[j]*y[j] for j in Facilities) + grb.quicksum(x[i,j] for i in Cities for j in Facilities)
